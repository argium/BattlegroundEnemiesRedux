...
local function overWriteSettingsKeepPlayerCount(t, k, v)
...
local function getOppositePlayerType(playerType)
  return playerType == "Enemies" and "Allies" or "Enemies"
end

local function getMatchingPlayerCountConfig(playerType, playerCountConfig)
  local oppositePlayerType = getOppositePlayerType(playerType)
  local oppositePlayerTypeConfig = BattleGroundEnemies.db.profile[oppositePlayerType]

  for i = 1, #oppositePlayerTypeConfig.playerCountConfigs do
    local oppositePlayerCountConfig = oppositePlayerTypeConfig.playerCountConfigs[i]
    if
      oppositePlayerCountConfig.minPlayerCount == playerCountConfig.minPlayerCount
      and oppositePlayerCountConfig.maxPlayerCount == playerCountConfig.maxPlayerCount
    then
      return oppositePlayerCountConfig, oppositePlayerType
    end
  end

  if oppositePlayerTypeConfig.CustomPlayerCountConfigsEnabled then
    for i = 1, #oppositePlayerTypeConfig.customPlayerCountConfigs do
      local oppositePlayerCountConfig = oppositePlayerTypeConfig.customPlayerCountConfigs[i]
      if
        oppositePlayerCountConfig.minPlayerCount == playerCountConfig.minPlayerCount
        and oppositePlayerCountConfig.maxPlayerCount == playerCountConfig.maxPlayerCount
      then
  ...
  return nil, oppositePlayerType
end

local function isInSameHorizontal(Point1, Point2)
  if type(Point1) ~= "string" or type(Point2) ~= "string" then return false end
  local p1 = string.match(Point1, "LEFT") or string.match(Point1, "RIGHT") or "CENTER"
  local p2 = string.match(Point2, "LEFT") or string.match(Point2, "RIGHT") or "CENTER"
...
function BattleGroundEnemies:AddModulesSettings(location, playerCountConfigDefault, playerType, condidtionFunc)
  ...
  for moduleName, moduleFrame in pairs(self.ButtonModules) do
    ...
    local oppositePlayerCountConfig, oppositePlayerType = getMatchingPlayerCountConfig(playerType, location)
    ...
    if condidtionFunc(moduleFrame) then
      temp[moduleName] = {
        ...
        args = {
          ...
          CopyFromOppositeFrame = {
            type = "select",
            name = L.CopyModuleFromOppositeFrame,
            desc = L.CopyModuleFromOppositeFrame_Desc:format(L[oppositePlayerType]),
            get = function()
              return ""
            end,
            set = function(option, value)
              local sourceModuleSettings = oppositePlayerCountConfig
                and oppositePlayerCountConfig.ButtonModules
                and oppositePlayerCountConfig.ButtonModules[moduleName]
              if not sourceModuleSettings then
                return
              end

              if value == "Normal" then
                location.ButtonModules[moduleName] = CopyTable(sourceModuleSettings)
              elseif value == "Mirrored" then
                location.ButtonModules[moduleName] =
                  BattleGroundEnemies:FlipButtonModuleSettingsHorizontally(moduleName, sourceModuleSettings)
              end
            ...
            disabled = function()
              return not (
                oppositePlayerCountConfig
                and oppositePlayerCountConfig.ButtonModules
                and oppositePlayerCountConfig.ButtonModules[moduleName]
              )
            end,
            confirm = function(option, value)
              local phrase = value == "Mirrored" and L.OverwriteMirroredConfirm or L.OverwriteNormalConfirm
              return phrase:format(
                moduleFrame.localizedModuleName .. " (" .. L[playerType] .. ")",
                moduleFrame.localizedModuleName .. " (" .. L[oppositePlayerType] .. ")"
              )
            end,
            width = "full",
            order = 4,
    ...
    settings[BattleGroundEnemies:GetPlayerCountConfigName(playerCountLocation)] = {
      ...
      args = {
        ...
        CopySettings = {
          ...
          set = function(option, value)
            overWriteSettingsKeepPlayerCount(
